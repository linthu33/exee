...
import '../dashboard/WidgetTree.dart';
...
class _VoucherScreenState extends State<VoucherScreen> {
  ...
  void initState() {
    ...
    selectedstatus = List.from(order_status);
  ...
  Widget buildInvoice(Orderproduct invoice) {
    var data = invoice.orderitem!.asMap().entries.map((item) {
      return DataRow(cells: [
        DataCell(Text(item.value.producttype.toString())),
        ...
        DataCell(Text((item.value.quantity! *
            ...
            .toString())),
        DataCell(
          DropdownButton<Order_DStatus>(
              dropdownColor: Color.fromARGB(255, 245, 135, 100),
              value: selectedstatus[item.key],
              style: const TextStyle(height: 1.5),
              icon: const Icon(Icons.keyboard_arrow_down),
              items: order_status
                  .map<DropdownMenuItem<Order_DStatus>>((Order_DStatus value) {
              ...
              onChanged: (Order_DStatus? newstatus) {
                setState(() {
                  ...
                  selectedstatus[item.key].ordItem = item.value.oid.toString();
                  //comfirmstatuspopup(item);
                  //print(selectedstatus[item.key].ordstatus);
                });
              }),
        ),
      ]);
...
class Order_DStatus {
  ...
  Order_DStatus({required this.ordstatus});
  ...
  static List<Order_DStatus> getUsers() {
    return <Order_DStatus>[
      ...
      Order_DStatus(ordstatus: "order"),
      Order_DStatus(ordstatus: "Cancel"),
      Order_DStatus(ordstatus: "Processing"),
      Order_DStatus(ordstatus: "Delivery"),
      Order_DStatus(ordstatus: "completed"),
    ];
  }
}
